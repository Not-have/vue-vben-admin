...
import type {
  ...
  RadioOption,
  SelectOption,
} from '@vben/mb/components/Form';
import type { Fn, Recordable } from '@vben/mb/types';

import { onMounted, reactive, ref } from 'vue';

import { BaseButton } from '@vben/mb/components/Button';
import { ContentWrap } from '@vben/mb/components/ContentWrap';
import { Form } from '@vben/mb/components/Form';
import { Icon } from '@vben/mb/components/Icon';
...
const schema = reactive<FormSchema[]>([
      ...
      slots: {
        default: (options: SelectOption[]) => {
          return options.length > 0
            ? options?.map((v) => {
                return (
                  <ElOption key={v.value} label={v.label} value={v.value} />
                );
              })
            : null;
        },
        ...
        default: ({ data, node }: any) => {
          return (
            <>
              ...
              {node.isLeaf ? null : <span> ({data.children.length}) </span>}
            </>
          );
        },
      },
    ...
    componentProps: {
      ...
      slots: {
        default: (options: RadioOption[]) => {
          return options?.map((v) => {
            return (
              <ElRadioButton label={`${v.label}(${v.value})`} value={v.value} />
            );
          });
        },
      },
    ...
    componentProps: {
      ...
      slots: {
        default: (options: CheckboxOption[]) => {
          return options?.map((v) => {
            return (
              <ElCheckbox label={`${v.label}(${v.value})`} value={v.value} />
            );
          });
        },
      },
  ...
  },
  {
    ...
    componentProps: {
      ...
      checkStrictly: true,
      checkOnClickNode: true,
      filterNodeMethod: (value: any, data: { label: any[] | string }) =>
  ...
  },
  {
    ...
    componentProps: {
      ...
      load: (
        ...
        resolve: (
          arg0: (
            | { isLeaf: boolean; label: string; value: number }
            | { isLeaf?: undefined; label: string; value: number }
          )[],
        ) => void,
      ) => {
  ...
  },
  {
    ...
    componentProps: {
      ...
      onExceed: (files: any[] | string, uploadFiles: any[] | string) => {
        ElMessage.warning(
  ...
  },
  {
    ...
    componentProps: {
      ...
      beforeUpload: (rawFile: { size: number; type: string }) => {
      ...
      slots: {
        default: () => (
          <>
            {imageUrl.value ? (
              <img class="avatar" src={imageUrl.value} />
            ) : null}
            {imageUrl.value ? null : (
