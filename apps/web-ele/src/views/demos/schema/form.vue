...
import type { ComponentSize, FormItemProp } from 'element-plus';
...
const formValidation = async () => {
  const elFormExpose = await getElFormExpose();
  elFormExpose?.validate((isValid: any) => {
    console.log(isValid);
  });
};
...
const inoutFocus = async () => {
  const inputEl: ComponentRef<typeof ElInput> =
    await getComponentExpose('field1');
  inputEl?.focus();
};
