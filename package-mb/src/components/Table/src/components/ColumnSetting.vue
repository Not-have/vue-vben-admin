...
const confirm = () => {
  const newColumns = cloneDeep(unref(settingColumns))?.map((item) => {
    const fixed = unref(settingColumns)?.find(
...
const initColumns = (columns: TableColumn[], isReStore = false) => {
  const newColumns = columns?.filter((item) => {
    if (!isReStore) {
      item.fixed = item.fixed === void 0 ? false : item.fixed;
    }
    return (
      (item.type && !DEFAULT_FILTER_COLUMN.includes(item.type)) || !item.type
    );
  });
  ...
  hiddenColumns.value = cloneDeep(
    columns?.filter(
      (item) => item.type && DEFAULT_FILTER_COLUMN.includes(item.type),
    ),
  );

  defaultCheckColumns.value =
    unref(settingColumns)?.map((item) => item.field) || [];
  checkColumns.value =
    ...
    <div>
      <div class="flex items-center justify-between">
        <div class="flex items-center justify-between">
          ...
          />
          <ElText class="ml-8px!">
            {{ checkColumns.length }} / {{ settingColumns?.length }}
          </ElText>
        </div>
        <ElText>固定 / 排序</ElText>
      </div>
          ...
          >
            ...
            >
              ...
              <div class="flex items-center">
                ...
                <div class="ml-12px handle cursor-move">
                  <Icon icon="ep:rank" />
                </div>
              </div>
            </div>
